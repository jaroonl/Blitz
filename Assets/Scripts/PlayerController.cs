...
public class PlayerController : MonoBehaviour
{
    ...
    public float jumpSpeed = 7f; // original jump speed
    ...
    void Start()
    {
        ...
        // freeze rotation on X and Z to prevent tipping over
    ...
    void WalkHandler()
    {
        // Get input on x ("Horizontal") and z ("Vertical")
        float hAxis = Input.GetAxis("Horizontal");
        float vAxis = Input.GetAxis("Vertical");

        // Movement vector scaled by speed and time
        Vector3 movement = new Vector3(hAxis, 0f, vAxis).normalized * walkSpeed * Time.deltaTime;

        // Move the rigid body to new position (avoiding direct velocity setting to prevent tipping)
    ...
    void JumpHandler()
    {
        ...
        if (jAxis > 0f)
        {
            ...
            if (!pressedJump && isGrounded)
            {
                ...
                pressedJump = true;
                // Jumping vector → jump scaled down by half
                Vector3 jumpVector = new Vector3(0f, jumpSpeed * 0.5f, 0f);

                // Make the player jump by adding velocity (keep current x, z)
